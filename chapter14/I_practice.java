...
//      3. 특정 부서(Finance) 직원 중 이름에 '조'가 포함된 직원 필터링
        List<Employee> employeesName = employees.stream()
                ...
                .collect(Collectors.toList());
        System.out.println("Finance 직원 중 이름에 '개'가 포함된 직원은: ");
...
//      5. 부서별 평균 급여 계산
        Map<String, Double> avgsa = employees.stream()
                .collect(Collectors.groupingBy(Employee::getDepartment,
                        Collectors.averagingDouble(Employee::getSalary)));

        System.out.println(avgsa);  // {Finance=8000.0, HR=6000.0, IT=6000.0}
